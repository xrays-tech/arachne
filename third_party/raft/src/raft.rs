...
// ---------------------------------------------------------------------------
// D-S1: seedable election RNG (Arachne test hook — see ARACHNE-PATCH.md).
//
// Upstream `raft` randomizes the election timeout with the process-wide
// `thread_rng`, which is unseedable and therefore makes the L2 double-run
// determinism gate (test-plan §5 E3) unprovable. This hook adds an OPTIONAL,
// thread-local seed: when a base seed is set (via `set_election_rng_seed`),
// each node draws its timeout from a per-node deterministic sub-stream seeded
// by `base_seed ^ node_id` (test-plan E3: one sub-stream per host). When no
// seed is set, the upstream `thread_rng` behavior is preserved EXACTLY, so
// production behavior is unchanged.
// ---------------------------------------------------------------------------
thread_local! {
    static ELECTION_RNG: RefCell<ElectionRng> = RefCell::new(ElectionRng::default());
}

/// Per-node deterministic sub-streams for the D-S1 seedable election RNG.
/// `base_seed == None` means "unseeded" (fall back to `thread_rng`).
#[derive(Default)]
struct ElectionRng {
    base_seed: Option<u64>,
    streams: HashMap<u64, rand::rngs::StdRng>,
}

/// D-S1 test hook: make the election-timeout RNG deterministic.
///
/// Sets the base seed and clears any per-node sub-streams. From now on, until
/// [`clear_election_rng_seed`] is called, each node's
/// `reset_randomized_election_timeout` draws from a per-node `StdRng` seeded by
/// `seed ^ node_id`, so a given seed reproduces the same election jitter. This
/// function is the ONLY D-S1 addition to `raft`'s public surface; it has no
/// effect on production behavior (the seed is never set outside tests).
pub fn set_election_rng_seed(seed: u64) {
    ELECTION_RNG.with(|rng| {
        let mut rng = rng.borrow_mut();
        rng.base_seed = Some(seed);
        rng.streams.clear();
    });
}

/// D-S1 test hook: restore the upstream unseeded (`thread_rng`) election RNG.
///
/// Clears the base seed and all per-node sub-streams.
pub fn clear_election_rng_seed() {
    ELECTION_RNG.with(|rng| {
        let mut rng = rng.borrow_mut();
        rng.base_seed = None;
        rng.streams.clear();
...
impl<T: Storage> Raft<T> {
    ...
    ///
    /// D-S1: when a base seed has been set via [`set_election_rng_seed`], the
    /// timeout is drawn from this node's deterministic sub-stream (seeded by
    /// `base ^ self.id`); otherwise the upstream `thread_rng` behavior is
    /// preserved unchanged.
    pub fn reset_randomized_election_timeout(&mut self) {
        let prev_timeout = self.randomized_election_timeout;
        let timeout = ELECTION_RNG.with(|rng| {
            let mut rng = rng.borrow_mut();
            match rng.base_seed {
                Some(base) => {
                    let stream = rng
                        .streams
                        .entry(self.id)
                        .or_insert_with(|| rand::rngs::StdRng::seed_from_u64(base ^ self.id));
                    stream.gen_range(self.min_election_timeout..self.max_election_timeout)
                }
