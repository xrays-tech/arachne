...
// Arachne D-S1 vendoring: `raft` 0.7.0 predates several lints added in the
// current toolchain (Rust 1.93). Suppressing them here (rather than editing the
// upstream source) keeps the vendored tree byte-identical except for the D-S1
// hook and keeps the workspace build warning-free. See ARACHNE-PATCH.md.
#![allow(
    unexpected_cfgs,
...
pub use crate::raft::{
    clear_election_rng_seed, set_election_rng_seed, vote_resp_msg_type, Raft, SoftState,
    StateRole, CAMPAIGN_ELECTION, CAMPAIGN_PRE_ELECTION, CAMPAIGN_TRANSFER, INVALID_ID,
    INVALID_INDEX,
};
