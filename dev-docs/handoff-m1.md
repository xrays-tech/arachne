...
- **T3 ✓ 已解决（第 16 轮）：根因在 raft 的"本地消息"分类，修法是 follower 侧本地重试**。
  ...
  - **剩余假设（收窄后）**：`served=0` 说明 leader **根本没有发起快照**（而不是发了没人接 ✗），而 victim 也没能用日志追上 ⇒ leader 到 victim 的**消息**可能整体没到。下一步最省事的做法：在这个 tonic harness 里给 leader 装一个探针，直接打印 (a) `RaftNode::peer_progress(victim)` ✓ 已存在的访问器（看 `state`/`next_idx`/`paused`/`recent_active`）、(b) leader 收到 victim 的 `MsgAppendResponse`/`MsgHeartbeatResponse` 的次数（沿用第 15 轮在 in-memory 里用过的同一手法 ✓）——这能立刻区分"消息没到"与"到了但 leader 不发快照"。
