...
- **T3 ✓ 已解决（第 16 轮）：根因在 raft 的"本地消息"分类，修法是 follower 侧本地重试**。
  ...
  - **卡在哪（诊断数据）**：全部节点起好、kill 一个 follower、写 240×1 KiB、重启后，victim 一直停在 `applied=2`，leader `commit=317`，**provider 调用次数 = 0**，victim 无快照文件 ⇒ **leader 从未把快照消息送到 victim**（或送到了但没被拦截）。已排除"能力没开"（断言 ✓）。下一个怀疑对象：leader 侧到 victim 的**缓存 gRPC channel**（在 victim 重启前建立、重启后指向已关闭的旧 listener ⇒ 发送失败、`dropped_sends` 增长、raft 反复重试而不重新建连 ✗）——建议下一步把 `dropped_sends` 暴露成 metric（或在此测试里调 `Transport` 直接发一条消息验证重连 ✓），再决定是修 transport 还是改测试。
