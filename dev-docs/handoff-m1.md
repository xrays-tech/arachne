...
- **M1 进行中**。已完成 M1-1、M1-2（含整改）、M1-3a 与 **(A) arachne-node 多进程 tonic 接线（commit 5081417）**；**未完成 M1-3b / M1-4 / M1-5**（(A) 已覆盖 M1-5 的 L3 前置与冒烟主体，(D) 的杀 leader / CLI 集成测试仍待做）。

### 提交线（新 → 旧）
```
5081417 M1-5 (A) arachne-node 真实 tonic 3 进程集群 + start_with_bind + config addresses  ← HEAD
...
### M1 验收对照（propsol §10）
① 杀 leader ≤2×election_timeout 出新主（→ M1-5/(D)）② 期间写返回 `NotLeader`/`QuorumUnavailable` 不挂死（**客户端面已在 `8cbc765` 证明**；(A) 已证多进程非 leader 写返回 503 不挂死；杀主窗口面待 (D)）③ hint 失效经 seeds 轮询恢复（Handle 已实现 seeds 兜底）④ 含切主窗口的 put/get 线性一致（→ M1-3b + M1-4）⑤ crate 文档首页语义表/错误矩阵（**已完成** `151066a`）。
