...
- **S1（缺陷 1+2 一起修）**：seam `Storage` 增 `save_conf_state`/`conf_change_index`，`WalStorage` 落 `membership` 文件（原子替换、CRC、I7 取用规则）+ 磁盘损坏 fail-start；apply 通道**按 `entry_type` 路由**，ConfChange 条目改走 `apply_conf_change` 并按 `conf_change_index` 跳过已应用条目；`install_snapshot` 路径的成员回灌。验证：手工构造一条 ConfChange 条目（`fault-injection`/测试专用入口）→ 提交 → **重启** → 成员保持（反向对照：不写 `0x04` 时退回 bootstrap，断言失败）；同一 ConfChange **回放两次** → ConfState 与 progress 不变、不报错。
