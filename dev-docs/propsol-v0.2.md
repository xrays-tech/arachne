...
- **真实 tonic 端到端验收 ✓（第 23 轮完成）**：改为**更小的 harness**——node 1 是唯一 voter（`bootstrap_voters = [1]`），node 2 以 learner 加入但**晚启动** ⇒ 无需 kill/重启（少掉 `AddrInUse`、factory 重绑、handle 重注册等一堆坑 ✓）。断言：leader 快照 > 64 KiB 上限 ✓、learner 安装快照 ✓、provider 被调用 ✓、最终追上 ✓（5.6s）。
  **该验收测试立刻抓出一个真 bug**：服务端按**固定 256 KiB** 分块，而 `max_message_size` 可配得更小（测试 64 KiB）⇒ **每个分块都超过上限**，服务端编码失败、客户端解码失败 ⇒ **上限配到 512 KiB 以下时流式快照永远传不过去** ✗（默认 8 MiB 下不触发，所以一直没暴露）。诊断链：`served=8`（leader 确实在发、learner 确实在拉 ✓）+ `fetch done result_ok=false`（传输中途失败 ✗）。修法：分块 = `min(SNAPSHOT_CHUNK_BYTES, max_message_size / 2)`（留一半给 gRPC/protobuf 帧头）。
- **（历史）真实 tonic 三节点回归（第 17 轮尝试）**：骨架（每节点一 factory + `start_with_bind` + 各自 provider + 64 KiB 上限 + `wal_trailing_keep_bytes = 0` + kill/重启）已跑通到「重启成功」，但 victim 停在 `applied=2` 而 **provider 调用次数 = 0** ⇒ leader 从未把快照送达；已排除「传输能力未开」（断言通过 ✓）。**第 18 轮已证伪**「缓存 gRPC channel 失连」：对照测试在**没有任何改动**的情况下也通过 ⇒ tonic 的 channel 会自行重连 ✓。收窄后的下一步是在 harness 里给 leader 装探针，打印 `RaftNode::peer_progress(victim)` 与收到的响应计数，以区分「消息没到」与「到了但 leader 不发快照」。做法与诊断数据见 handoff，重建很快。
- **（历史记录，第 13–14 轮）**
